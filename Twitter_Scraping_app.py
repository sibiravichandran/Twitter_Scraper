...
def download_json(tweets_df): 
    json_data = tweets_df.to_json(orient='records').encode('utf-8')
    return json_data

#The to_json method of the tweets_df DataFrame is used to convert the data to a JSON-formatted string with each row represented as a separate record in a list. 
#The encode method is then called on this string to convert it to a byte string encoded in UTF-8 format.
